...
os.system("python3 -m pip install pypisearch")
...
def execute_pip_command(command):
    try:
        result = subprocess.check_output(f"python3 -m pip {command}", stderr=subprocess.STDOUT, shell=True, text=True)
...
def search():
    ...
    try:
        result = subprocess.check_output(f"python3 -m pypisearch {name}", stderr=subprocess.STDOUT, shell=True, text=True)
    except subprocess.CalledProcessError as e:
        result = e.output
    output_text.configure(state='normal')
    output_text.delete("1.0", tk.END)
